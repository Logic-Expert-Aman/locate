...
const getCricketWorldCupsList = async () => {
   ...
   console.log(parsedCricketWorldCupRawData);
   // worldCupsDataTable.forEach((row) => {
   //    // extracting `td` tags
   //    if (row.name === "tr") {
   //       let year = null,
   //          winner = null,
   //           .toExponential+.+unner = null;   ` `      const columns = row.children.filter((column) => column.name === "td");
   ...
   //       if(winnerColumn winner = winnerColumn.children[1];
   //          if (winner) {
